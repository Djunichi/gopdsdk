...
uintptr_t bridgeNewVideoStream(void){return(uintptr_t)activePlaydate->graphics->videostream->newPlayer();}void bridgeFreeVideoStream(uintptr_t p){activePlaydate->graphics->videostream->freePlayer((LCDStreamPlayer*)p);}void bridgeVideoStreamSetBufferSize(uintptr_t p,int32_t v,int32_t a){activePlaydate->graphics->videostream->setBufferSize((LCDStreamPlayer*)p,(int)v,(int)a);}void bridgeVideoStreamSetFile(uintptr_t p,uintptr_t f){activePlaydate->graphics->videostream->setFile((LCDStreamPlayer*)p,(SDFile*)f);}uintptr_t bridgeVideoStreamVideoPlayer(uintptr_t p){return(uintptr_t)activePlaydate->graphics->videostream->getVideoPlayer((LCDStreamPlayer*)p);}int32_t bridgeVideoStreamUpdate(uintptr_t p){return activePlaydate->graphics->videostream->update((LCDStreamPlayer*)p);}int32_t bridgeVideoStreamBufferedFrames(uintptr_t p){return activePlaydate->graphics->videostream->getBufferedFrameCount((LCDStreamPlayer*)p);}uint32_t bridgeVideoStreamBytesRead(uintptr_t p){return activePlaydate->graphics->videostream->getBytesRead((LCDStreamPlayer*)p);}
