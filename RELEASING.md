...
The first public candidate is `v0.1.0`, covering P1.0 through P1.4. Go module versions
come from Git tags; no separate VERSION file is maintained.

## Candidate gates

1. Confirm the worktree contains only reviewed release changes.
   Check `git show-ref --tags v0.1.0` as well: a stale local tag from the
...
3. Observe green native CI on Windows, macOS, and Linux, including the external
   `v0.1.0` consumer and Linux race detector.
...
6. Review [API.md](API.md), [COMPATIBILITY.md](COMPATIBILITY.md), README commands,
   public Go documentation, and release notes for unsupported claims.

## Version and consumer check

Before the tag exists, acceptance uses `require ... v0.1.0` with a local
...
go get github.com/Djunichi/gopdsdk@v0.1.0
