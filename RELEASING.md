...
The current candidate is `v0.4.0`, covering P4 on top of `v0.3.0`. Go module
...
1. Confirm the worktree contains only reviewed release changes.
   Check `git show-ref --tags v0.4.0` as well: a stale local tag from the
...
5. On the physical device, exercise new run, save checkpoint, normal restart,
   continue/load, persisted System Menu settings, application update, corrupt
   save handling, and Launcher exit. Record every injected or skipped failure
   scenario separately.
6. Run the required 60-second physical regression soak and compare both device
   logs with their pre-run sizes and timestamps only when crashlog inspection
   was explicitly requested for the release run.
7. Review [API.md](API.md), [COMPATIBILITY.md](COMPATIBILITY.md),
   [MIGRATING.md](MIGRATING.md), README commands,
   public Go documentation, and [CHANGELOG.md](CHANGELOG.md) for unsupported
   claims. Confirm the API snapshot changed only by the intended P4 additions.

## Version and consumer check

Before the tag exists, acceptance uses `require ... v0.4.0` with a local
...
go get github.com/Djunichi/gopdsdk@v0.4.0
go run github.com/Djunichi/gopdsdk/cmd/gopdsdk@v0.4.0 doctor
```
