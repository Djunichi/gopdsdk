...
The current candidate is `v0.2.0`, covering P2.0 through P2.5 on top of
`v0.1.1`. Go module versions come from Git tags; no separate VERSION file is
maintained.

## Candidate gates

1. Confirm the worktree contains only reviewed release changes.
   Check `git show-ref --tags v0.2.0` as well: a stale local tag from the
...
6. Review [API.md](API.md), [COMPATIBILITY.md](COMPATIBILITY.md), README commands,
   ...
   claims. Confirm the API snapshot changed only by the intended P2 additions.

## Version and consumer check

Before the tag exists, acceptance uses `require ... v0.2.0` with a local
...
go get github.com/Djunichi/gopdsdk@v0.2.0
